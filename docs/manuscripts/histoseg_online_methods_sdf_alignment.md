...
\rightarrow
\text{hard-alignment tournament}
\rightarrow
\text{topology-safe TPS}
\rightarrow
...
The first step is a relationship-mapping and interpretation layer. HistoSeg
consumes selected or curated structure groups, including groups prepared from
upstream sfplot StructureMap interpretation, and converts them into continuous
semantic geometry from the observed cell or transcript coordinates.

This establishes a boundary between semantic definition and registration.
StructureMap analysis is upstream evidence for selecting structure groups;
semantic contours are the geometry consumed by HistoSeg 3D; and the optional
CODA-inspired backend is a downstream hard-alignment seed candidate evaluated
after contours have been generated. The CODA-inspired backend therefore cannot
alter structure definitions, only the initial pose used before topology-safe TPS
refinement.
